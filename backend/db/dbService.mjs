...
export class UserCredDbService {
    ...
    async searchLengthUserCred(types, natures, fields, values){
        try{
            const resp = await new Promise((resolve, reject) => {
                if(types[0] == 'LIKE') values[0] = `%${values[0]}%`;
                let query = `SELECT COUNT(id) as length FROM usersCred WHERE ${fields[0]} ${types[0]} '${values[0]}'`;
...
export class ProjectsDbService {
    ...
    async searchLengthProjectsDetails(types, natures, fields, values){

        try{
            const resp = await new Promise((resolve, reject) => {
                if(types[0] == 'LIKE') values[0] = `%${values[0]}%`;
                let query = `SELECT COUNT(id) as length FROM projectsDetails WHERE ${fields[0]} ${types[0]} '${values[0]}'`;
...
export class TasksDbService {
    ...
    async searchLengthTasks(types, natures, fields, values){

        try{
            const resp = await new Promise((resolve, reject) => {
                if(types[0] == 'LIKE') values[0] = `%${values[0]}%`;
                let query = `SELECT COUNT(tasks.id) as length FROM tasks JOIN usersCred ON tasks.userId = usersCred.id 
                             LEFT JOIN usersCred as secondJoin ON tasks.userAssignedId = secondJoin.id WHERE tasks.${fields[0]} ${types[0]} '${values[0]}'`;
